...
- **계산에 사용하는 기본 지표**: 
  - DART API: `operating_income` (영업이익)
...
- **현재 계산에 사용하지 않는 지표**: 
  - `total_assets`, `net_income`, `current_liabilities`, `interest_bearing_current_liabilities`, `interest_expense` (금융비용으로 대체됨)
  - 고정값: `beta` (1.0), `mrp` (5.0)
- **계산된 지표**: `fcf`, `icr` (주석 처리), `roic`, `wacc`
...
- `collect_company_data()`: 회사 데이터 통합 수집
  ...
  - XBRL 데이터 수집 (재무상태표 및 현금흐름표 지표)
  - `CompanyFinancialObject` 객체 생성 및 채우기
  - 계산 지표 자동 계산 (`IndicatorCalculator.calculate_all_indicators()` 통합)
  - (추후) DB 저장 로직
...
- `calculate_icr()`: ICR (이자보상비율) 계산 (현재 주석 처리: 이자비용 데이터 부족)
- `calculate_roic()`: ROIC (투하자본수익률) 계산
- `calculate_wacc()`: WACC (가중평균자본비용) 계산
  - 이자비용 대신 금융비용 사용 (ROIC와 비교 시 가산 수치 필요)
- `calculate_all_indicators()`: 모든 계산 지표를 한 번에 계산
...
- **통합 계산**: `DataOrchestrator`에서 데이터 수집 후 자동으로 재무 지표 계산 수행
- **FCF 계산 최적화**: CAPEX 절대값 처리로 자산 처분 등 음수 케이스 대응
- **금융비용 사용**: WACC 계산에서 이자비용 대신 금융비용 사용 (ROIC와 비교 시 가산 수치 필요)
