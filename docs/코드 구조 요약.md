...
- **DartDataService**: 연간/분기 재무제표 수집, 기본 지표 채우기(매핑표·정규화). **사업보고서 1건당 당기·전기·전전기(thstrm, frmtrm, bfefrmtrm) 3개 연도 추출**, 이미 존재 연도 스킵(중복 방지). CFS 우선, 분기 병렬(ThreadPoolExecutor). DART 금액 "-" 등 비정상 값은 0으로 처리(ValueError 방지).
...
- **DB 저장**: SQLite. 재수집 판단: 4월 1일 기준 + yearly_data 없음 또는 모든 매출액 0이면 수집.
