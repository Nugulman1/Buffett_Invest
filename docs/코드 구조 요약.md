...
│   │   ├── paste_parser.py # 복붙 재무제표 파서 (연도 추출, 표 본문 자산/영업 분리, 줄=셀·빈줄=빈셀, 3연도 슬롯 매핑 → rows JSON)
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회), `passed_json`, `memo`, `paste_parser`(연결 재무상태표/현금흐름표: 연도 추출·표 본문 자산/영업 분리·줄 단위 셀·빈 줄=빈 셀 보존·값 개수+앞/가운데/뒤 빈 칸으로 3연도 슬롯 매핑 → `years`·`data`·`rows` 반환. 프로젝트 루트 `debug_paste_parser_balance.py`·`debug_paste_parser_cash.py`로 검증).
