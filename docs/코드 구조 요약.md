...
│   │   ├── paste_parser.py  # 복붙 재무제표 파서 (연도·단위 추출, 표 본문 자산/영업 분리, rows JSON, 원 단위 정규화)
│   │   ├── llm_extractor.py # LLM 기반 rows → 지표 추출 (cfo, 유형/무형자산취득, 기말현금, 이자비용, 이자부채). 필드별 가이드·이자부채 5계정 합산·interest_bearing_debt_breakdown 디버깅
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회), `passed_json`, `memo`, `paste_parser`(연도·단위 추출, 표 본문 분리, rows JSON, 원 단위 정규화), `llm_extractor`(rows → cfo·이자부채 등 6개 필드 추출, OpenAI 사용. 필드별 지표 가이드, 이자부채=단기차입금+장기차입금+유동성장기차입금+사채+리스부채, interest_bearing_debt_breakdown으로 디버깅 시 계정→금액 로그 출력).
