...
- **`Company`**: 회사 정보 및 필터 결과를 저장하는 Django 모델
  ...
  - 필터 결과: `passed_all_filters`, `filter_operating_income`, `filter_net_income`, `filter_revenue_cagr`, `filter_total_assets_operating_income_ratio`, `filter_roe`
  - `memo` (메모, 기업 분석 내용 저장)
...
- **필터 결과**: 
  ...
  - `filter_operating_income`, `filter_net_income`, `filter_revenue_cagr`, `filter_total_assets_operating_income_ratio`, `filter_roe` (개별 필터 통과 여부)
...
- `save_company_to_db()`: CompanyFinancialObject를 Django 모델로 변환하여 DB에 저장
  ...
  - DataOrchestrator에서 사용 (이전 `_save_to_db()` 메서드를 유틸 함수로 분리)
  - `filter_roe` 필드 포함
...
- `save_manual_financial_data()`: 수기 재무 데이터 저장 API
  - 수기 입력 데이터 저장, 자동 계산 (총자산영업이익률, ROE), 필터 재검사
...
- **`list.html`**: 기업 조회 페이지
  ...
  - 즐겨찾기 섹션: 그룹별 즐겨찾기 목록 표시, 그룹 필터링, 그룹 추가/수정/삭제 기능
- **`detail.html`**: 기업 상세 정보 페이지
  - 레이아웃: 2x2 그리드 구조
    - 상단 왼쪽: 기업정보 (회사명, 업종, 기업코드, 업종코드, 국채수익률, 사업보고서 링크)
    - 상단 오른쪽: 필터 통과 여부 (전체 필터 통과 여부, 개별 필터 결과)
    - 하단 왼쪽: 각종 지표 (최근 5년 재무 지표 테이블, 폰트 크기 0.8rem)
    - 하단 오른쪽: 즐겨찾기 (그룹별 즐겨찾기 목록, 즐겨찾기 추가 기능)
    - 맨 아래: 메모 (전체 너비)
  - 회사 기본 정보
  - 필터 결과 시각화 (영업이익, 당기순이익, 매출액 CAGR, 총자산영업이익률, ROE 필터)
  - 메모 작성 및 저장 기능 (메모 마지막 수정 날짜 표시)
  - 최근 5년 재무 지표 테이블 (FCF, ROIC, WACC 포함)
  - 사업보고서 링크 (DART 공시시스템)
...
- **`calculator.html`**: 재무 지표 계산기 페이지
  ...
  - 실시간 계산 (Alpine.js)
  - 기업코드 입력 필드 포함 (전역)
  - 각 년도별 "DB에서 가져오기" 버튼: 해당 년도의 자기자본, 영업이익, 국채수익률 자동 채우기
...
- **필터링 시스템**: 5가지 필터 조건으로 장기 투자 대상 기업 선별
  - 영업이익 필터, 당기순이익 필터, 매출액 CAGR 필터 (≥5%), 총자산영업이익률 필터, ROE 필터 (기업 규모별)
...
- **즐겨찾기 기능**: 기업을 그룹별로 분류하여 즐겨찾기 관리
  - 그룹 생성/수정/삭제 기능
  - 기업을 그룹에 추가/삭제 기능
  - 기업 상세 페이지와 조회 페이지에서 즐겨찾기 목록 표시
