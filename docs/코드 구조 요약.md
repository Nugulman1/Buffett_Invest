...
- **ORM 모델**: `Company` (corp_code PK, 필터 결과, 메모, last_collected_at), `BondYield` (단일 행, 하루 캐시), `ApiCallStats` (일별 호출 통계), `YearlyFinancialData` (연도별 재무, company+year unique. 기본: revenue·operating_income·net_income·total_assets·total_equity·operating_margin·roe. **다중회사 확장**: current_assets·noncurrent_assets·current_liabilities·noncurrent_liabilities·total_liabilities·capital_stock·retained_earnings·profit_before_tax), `YearlyFinancialIndicator` (회사·연도·지표코드·지표명 idx_nm·지표값 idx_val. DART fnlttCmpnyIndx 수집), `QuarterlyFinancialData` (분기별), `FavoriteGroup`, `Favorite` (group-company unique).
