...
│   │   ├── krx_client.py    # KRX OPEN API (AUTH_KEY). 유가(stk_bydd_trd)·코스닥(ksq_bydd_trd)·코넥스(knx_bydd_trd) 시장별 JSON 스냅샷(data/krx_daily_snapshot*.json) 저장·병합·캐시. 조회 시 유가→코스닥→코넥스 순 검색(break). 하루 전·이틀 전 순 조회. 재수집: 시장별 파일 없음 또는 bas_dd≠오늘&08:30 KST 이상. fetch_and_save_company_market_cap → Company.market_cap 갱신.
│   │   ├── memo.py          # 기업 메모 백업/복원
...
- **krx_client**: KRX 일별매매정보 **유가(stk_bydd_trd)·코스닥(ksq_bydd_trd)·코넥스(knx_bydd_trd)** 3시장. BASE_URL 고정 `https://data-dbg.krx.co.kr`. 당일이 아닌 **하루 전→이틀 전** 순으로 조회 후 시장별 JSON 스냅샷(`data/krx_daily_snapshot.json`, `krx_daily_snapshot_kosdaq.json`, `krx_daily_snapshot_konex.json`) 저장. 재수집: 시장별 파일 없음 또는 bas_dd≠오늘(한국날짜)이고 08:30 KST 이상. `ensure_latest_snapshot()` 시 3파일 로드·병합(유가+코스닥+코넥스 순) 반환. `get_snapshot_row_by_isu_cd(isu_cd)`는 병합 rows에서 유가→코스닥→코넥스 순 검색·첫 매칭 반환(break). API 미수집 시 기존 파일 병합 폴백. `fetch_and_save_company_market_cap(corp_code)` → Company.market_cap 갱신. 관리 명령 `fetch_krx_daily`.
