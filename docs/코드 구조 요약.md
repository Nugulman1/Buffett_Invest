...
- `calculate_basic_financial_ratios_for_quarterly()`: 분기보고서용 기본 재무지표 계산
  - 분기 데이터에 대해 영업이익률과 ROE 계산
  - `calculate_operating_margin()`, `calculate_roe()` 함수 재사용
...
- **기능**: 
  - `YearlyFinancialData`만 삭제 (Company는 유지하여 메모 보존)
  - 필터 필드 초기화 (`passed_all_filters`, `filter_*` 모두 False)
  - `last_collected_at` 초기화 (재수집 가능하게)
  - 전체 삭제 시 `passed_filters_stock_codes.txt` 파일도 초기화
- **주의**: `--confirm` 옵션 없이 실행하면 미리보기만 표시
...
- **재수집 관리**: `clear_financial_data` 명령어로 재무지표 데이터만 삭제하여 재수집 가능 (메모 보존, 필터 필드 자동 초기화)
