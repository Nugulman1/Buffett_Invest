...
│   │   ├── paste_parser.py  # 복붙 재무제표 파서 (연도·단위 추출, 표 본문 자산/영업 분리, rows JSON, 원 단위 정규화). parse_balance_sheet/parse_cash_flow 반환 직전 파싱 결과 전체 JSON을 print로 콘솔 출력
│   │   ├── llm_extractor.py # LLM 기반 rows → 지표 추출 (cfo, 유형/무형자산취득, 기말현금, 이자비용, 이자부채). 연도별 _build_prompt 동적 생성. CFO 부호 유지 프롬프트. 이자부채는 LLM 총합 대신 interest_bearing_debt_breakdown 합으로 _sum_breakdown 계산(합산 오류 방지). 6계정(단기/장기차입금, 유동성장기차입금, 사채, 리스부채, 비유동리스부채)
...
- **DartDataService**: 연간/분기 재무제표 수집, 기본 지표 채우기(매핑표·정규화). **사업보고서 1건당 당기·전기·전전기(thstrm, frmtrm, bfefrmtrm) 3개 연도 추출**, 이미 존재 연도 스킵(중복 방지). CFS 우선, 분기 병렬(ThreadPoolExecutor). DART 금액 "-" 등 비정상 값은 **None**(데이터 없음) 저장.
...
- **재무·계산기·분기·메모**: `/api/companies/<corp_code>/financial-data/`, `calculator-data/`, `parse-paste/`(POST, 복붙→paste_parser→LLM 추출→FCF/ROIC/WACC 계산·DB 저장), `quarterly-data/`, `memo/`, `calculated-indicators/`, `quarterly-reports/collect/`, `annual-report-link/`.
...
- **config/settings.py**: INSTALLED_APPS(rest_framework, apps, dart, ecos, companies), DB(SQLite, 타임아웃 30초), API 키(.env), OPENAI_API_KEY·OPENAI_MODEL, DATA_COLLECTION, LOGGING.
- **.env** (필수) 예시:
```
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회), `passed_json`, `memo`, `paste_parser`(연도·단위 추출, 표 본문 분리, rows JSON, 원 단위 정규화. 반환 직전 파싱 결과 전체 JSON을 print로 콘솔 출력), `llm_extractor`(rows → cfo·이자부채 등 6개 필드 추출, OpenAI 사용. CFO 부호 유지 프롬프트. 이자부채는 LLM 총합 대신 interest_bearing_debt_breakdown을 _sum_breakdown으로 합산해 사용. 6계정: 단기/장기차입금, 유동성장기차입금, 사채, 리스부채, 비유동리스부채. 디버깅 시 breakdown 계정→금액 로그 출력).
