...
│   │   ├── llm_extractor.py # LLM 기반 rows → 지표 추출 (cfo, 유형/무형자산취득, 기말현금, 이자비용, 이자부채, dividend_paid 배당금지급). 연도별 _build_prompt 동적 생성. CFO 부호 유지. 이자부채: breakdown 합 _sum_breakdown. _log_extracted_indicators 로그. OPENAI_MODEL 기본 gpt-4o
...
│   │   ├── 0005_add_idx_nm_to_yearly_financial_indicator.py # YearlyFinancialIndicator.idx_nm 지표명
│   │   └── 0006_add_dividend_paid_to_yearly_financial.py # YearlyFinancialData.dividend_paid (배당금지급)
...
- **ORM 모델**: `Company` (corp_code PK, 필터 결과, 메모, last_collected_at), `BondYield` (단일 행, 하루 캐시), `ApiCallStats` (일별 호출 통계), `YearlyFinancialData` (연도별 재무, company+year unique. 기본: revenue·operating_income·net_income·total_assets·total_equity·operating_margin·roe·interest_bearing_debt·**dividend_paid**(배당금지급, LLM 추출)·fcf·roic·wacc. **다중회사 확장**: current_assets·noncurrent_assets·current_liabilities·noncurrent_liabilities·total_liabilities·capital_stock·retained_earnings·profit_before_tax), `YearlyFinancialIndicator` (회사·연도·지표코드·지표명 idx_nm·지표값 idx_val. DART fnlttCmpnyIndx 수집), `QuarterlyFinancialData` (분기별), `FavoriteGroup`, `Favorite` (group-company unique).
...
- **IndicatorCalculator**: CAGR, 영업이익률, FCF, ROIC, WACC 계산. 비율은 소수 저장(0.10=10%), UI에서만 *100. **ROE는 계산하지 않음** — DART 주요재무지표 M211550에서 채워서 yearly_data.roe에 저장. **영업이익률**은 데이터 없으면 None 반환. **FCF/ROIC/WACC**: DB의 영업이익·자기자본(total_equity)이 None이면 해당 연도는 계산 생략.
...
- **재무·계산기·분기·메모**: `/api/companies/<corp_code>/financial-data/`(yearly_data에 dividend_paid 포함), `calculator-data/`, `parse-paste/`(POST, 복붙→paste_parser→LLM 추출→FCF/ROIC/WACC·dividend_paid 계산·DB 저장), `quarterly-data/`, `memo/`, `calculated-indicators/`, `quarterly-reports/collect/`(POST, 최근 3개 분기보고서 수집, 사업보고서 날짜 무관), `annual-report-link/`.
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회. **save_company_to_db**: YearlyFinancialData 확장 필드 8개 + dividend_paid + YearlyFinancialIndicator 저장. **load_company_from_db**: yearly_data_obj.dividend_paid 로드. **분기 수집**: `get_company_for_quarterly_collect`는 기업만 반환), `passed_json`, `memo`, `paste_parser`(연도·단위 추출, 표 본문 분리, rows JSON, 원 단위 정규화. 반환 직전 _format_parsed_display로 표 형태 출력), `llm_extractor`(rows → cfo·이자부채·dividend_paid 등 7개 필드 추출, OpenAI gpt-4o 기본. CFO 부호 유지. 이자부채 breakdown _sum_breakdown. _log_extracted_indicators 로그).
