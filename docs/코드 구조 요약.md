...
│   │   ├── xbrl_disabled.py # (비활성) XBRL ACODE 추출 코드 보관
...
- XBRL ACODE 기반 추출 로직은 표본 부족으로 중단되었고, 관련 코드는 `apps/service/xbrl_disabled.py`로 분리해 보관
...
- `get_bond_yield_5y()`는 **소수 형태로 반환** (예: 0.03057 = 3.057%)
...
- `collect_quarterly_reports()`: 최근 분기보고서 수집 API
  - DB에 저장된 가장 최근 사업보고서(접수일/연도) 이후의 분기보고서를 수집하여 DB에 저장 (연간 재무 수집 후 사용)
  - rcept_no→reprt_code dict 조회, (year, quarter) 최신순 정렬 후 저장/응답
