...
│   │   ├── krx_client.py    # KRX OPEN API (AUTH_KEY). 당일 전체 종목 JSON 스냅샷(data/krx_daily_snapshot.json) 저장·캐시, ISU_CD로 조회, fetch_and_save_company_market_cap → Company.market_cap만 갱신. 재수집: 파일 없음 또는 bas_dd≠오늘&08:30 KST 이상
│   │   ├── memo.py          # 기업 메모 백업/복원
...
- **krx_client**: KRX 유가증권 일별매매정보(OPPUSES002_S2). 당일 전체 종목 1회 호출(basDd만) → **JSON 스냅샷** 1파일(`data/krx_daily_snapshot.json`, 15개 필드) 저장·메모리 캐시. 재수집: JSON 없음 또는 bas_dd≠오늘(한국날짜)이고 현재 시각 08:30 KST 이상. `ensure_latest_snapshot()`, `get_snapshot_row_by_isu_cd(isu_cd)`, `fetch_and_save_company_market_cap(corp_code)` → 스냅샷에서 ISU_CD로 조회 후 Company.market_cap만 갱신. 관리 명령 `fetch_krx_daily` (08:30 스케줄용).
...
- **config/settings.py**: INSTALLED_APPS(rest_framework, apps, dart, ecos, companies), DB(SQLite, 잠금 대기 **타임아웃 10초**), API 키(.env), OPENAI_API_KEY·OPENAI_MODEL(기본 gpt-4o), **KRX_API_KEY·KRX_BASE_URL·KRX_DAILY_SNAPSHOT_PATH**(당일 스냅샷 JSON 경로), **SECOND_FILTER_ROIC_WACC_SPREAD**(2차 필터, 기본 0.02), CALCULATOR_DEFAULTS, **DATA_COLLECTION**(PARALLEL_WORKERS 2 이상 시 스레드별 DB 연결로 SQLite locked 가능 → save_company_to_db 재시도로 완화), LOGGING.
- **.env** (필수/선택) 예시:
```
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `krx_client`(KRX 당일 전체 JSON 스냅샷 저장·캐시·ISU_CD 조회·Company.market_cap 갱신), `db`(연간/분기 저장·조회, save_company_to_db에 locked 재시도; save 시 cash_and_cash_equivalents·noncontrolling_interest·ev·invested_capital·selling_admin_expense_ratio 포함), `memo`, `paste_parser`, `llm_extractor`(rows → cfo·이자부채·dividend_paid·noncontrolling_interest 등 8개 필드 추출).

### 7. 프론트엔드 (`apps/companies/`)

- **뷰**: pages(목록·계산기·상세), api_financial(재무·계산기·분기·메모), api_favorites(즐겨찾기·그룹), api_misc(검색·passed).
- **템플릿**: base(Bootstrap 5 + Alpine.js). list — 목록·검색(기업명/종목코드 6자리/기업번호 8자리)·즐겨찾기·필터 통과 목록. detail — 상세·필터 결과·**각종 지표 테이블**(연도, 매출액, 영업이익, 당기순이익, 영업이익률, ROE, 판관비율, FCF, ROIC, WACC, EV/IC, 부채비율, 배당성향, 배당지급액, 시가총액; 가로·세로 스크롤)·메모·재무표·즐겨찾기. calculator — 국채수익률 페이지 로드 시 ECOS BondYield 자동 채움, 연결 재무상태표·현금흐름표 복붙 → parse-paste API.
...
- **데이터 소스**: DART·ECOS·KRX API(유가증권 일별매매정보 당일 전체 1회 호출 → JSON 스냅샷 저장, ISU_CD로 시가총액 등 조회).
