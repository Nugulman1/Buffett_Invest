...
│   │   ├── llm_extractor.py # LLM 기반 rows → 지표 추출 (cfo, 유형/무형자산취득, 기말현금, 이자비용, 이자부채)
...
- **IndicatorCalculator**: CAGR, 영업이익률·ROE, FCF, ROIC, WACC 계산. 비율은 소수 저장(0.10=10%), UI에서만 *100. **영업이익률·ROE는 데이터 없으면 None 반환** (revenue/total_equity None 또는 0 이하 시).
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회), `passed_json`, `memo`, `paste_parser`(연도·단위 추출, 표 본문 분리, rows JSON, 원 단위 정규화), `llm_extractor`(rows → cfo·이자부채 등 6개 필드 추출, OpenAI 사용).
