...
- **재무·계산기·분기·메모**: `/api/companies/<corp_code>/financial-data/`, `calculator-data/`, `quarterly-data/`, `memo/`, `calculated-indicators/`, `quarterly-reports/collect/`, `annual-report-link/`.
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회), `passed_json`, `memo`.

### 8. 프론트엔드 (`apps/companies/`)

- **뷰**: pages(목록·계산기·상세), api_financial(재무·계산기·분기·메모), api_favorites(즐겨찾기·그룹), api_misc(검색·passed).
- **템플릿**: base(Bootstrap 5 + Alpine.js). list — 목록·검색·즐겨찾기·필터 통과 목록(페이지네이션). detail — 상세·필터 결과·메모·재무표·즐겨찾기. calculator — 다년도 FCF/ROIC/WACC 입력·자동 채우기·저장.
