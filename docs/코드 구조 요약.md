...
- **ORM 모델**: `Company` (corp_code PK, 필터 결과, 메모, last_collected_at), `BondYield` (단일 행, 하루 캐시), `ApiCallStats` (일별 호출 통계), `YearlyFinancialData` (연도별 재무, company+year unique, **revenue·operating_income·net_income·total_assets·total_equity·operating_margin·roe null=True**), `QuarterlyFinancialData` (분기별, company+year+quarter unique), `FavoriteGroup`, `Favorite` (group-company unique).
...
- **DartDataService**: 연간/분기 재무제표 수집, 기본 지표 채우기(매핑표·정규화). **사업보고서 1건당 당기·전기·전전기(thstrm, frmtrm, bfefrmtrm) 3개 연도 추출**, 이미 존재 연도 스킵(중복 방지). CFS 우선, 분기 병렬(ThreadPoolExecutor). DART 금액 "-" 등 비정상 값은 **None**(데이터 없음) 저장. **분기 수집**: `collect_quarterly_data_for_save(corp_code, limit=3)` — `get_recent_quarterly_reports` 호출(사업보고서 날짜 무관, 최근 3개 분기).
...
- **service**: `corp_code`(종목코드↔기업번호), `bond_yield`(BondYield 캐시), `db`(수집 여부·연간/분기 저장·조회. **분기 수집**: `get_company_for_quarterly_collect`는 기업만 반환, after_date 없음), `passed_json`, `memo`, `paste_parser`(연도·단위 추출, 표 본문 분리, rows JSON, 원 단위 정규화. 반환 직전 _format_parsed_display로 표 형태 출력), `llm_extractor`(rows → cfo·이자부채 등 6개 필드 추출, OpenAI gpt-4o 기본. CFO 부호 유지. 이자부채 breakdown은 rows 계정명 키, _sum_breakdown 합산. _log_extracted_indicators 로그).
