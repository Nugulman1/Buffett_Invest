...
- **기본 지표 (DART API)**: 
  ...
  - `total_assets_operating_income_ratio` (총자산영업이익률) - 계산 방식 (영업이익/자산총계)
  - `roe` (ROE) - 계산 방식 (당기순이익/자본총계)
- **현재 사용하지 않는 지표 (XBRL 관련, 주석 처리됨)**: 
  - XBRL 데이터 수집이 중단되어 다음 지표들은 더 이상 수집되지 않음:
  - `tangible_asset_acquisition`, `intangible_asset_acquisition`, `cfo`, `equity`, `cash_and_cash_equivalents`, `short_term_borrowings`, `current_portion_of_long_term_borrowings`, `long_term_borrowings`, `bonds`, `lease_liabilities`, `finance_costs`
  - `current_liabilities`, `interest_bearing_current_liabilities`, `interest_expense`
  - 고정값: `beta` (1.0), `mrp` (5.0)
- **계산된 지표**: 현재 사용하지 않음 (XBRL 데이터 수집 중단으로 인해 모든 계산 지표 계산이 주석 처리됨)
  - `fcf`, `roic`, `icr`, `wacc` 모두 주석 처리됨
...
- `calculate_fcf()`, `calculate_icr()`, `calculate_roic()`, `calculate_wacc()`: 계산 메서드는 존재하나 사용하지 않음
- `calculate_all_indicators()`: **주석 처리됨 (사용하지 않음)**
  - XBRL 데이터 수집이 중단되어 모든 계산 지표 계산이 주석 처리됨
  - FCF, ICR, ROIC, WACC 계산에 필요한 데이터(CFO, 금융비용, 자기자본 등)는 XBRL에서만 수집 가능
  - 표본이 너무 적어서 데이터화를 못할듯하여 일단 중단
