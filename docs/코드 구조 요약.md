...
- **IndicatorCalculator**: CAGR, 영업이익률, **부채비율**(자본총계/부채총계), FCF, ROIC, WACC, **투하자본(IC)**, **EV**(시총+이자부채-(기말현금및현금성자산×70%)+비지배지분, 현금 보수적 반영) 계산. 비율은 소수 저장. ROE는 DART M211550에서 채움. **WACC**: Re 계산 시 국채수익률에 0.5%p 보수적 가산 적용. **FCF/ROIC/WACC**: DB 영업이익·자기자본 None이면 해당 연도 계산 생략.
