...
- **기본 지표**: 
  - DART API: `total_assets`, `operating_income`, `net_income`, `current_liabilities`, `interest_bearing_current_liabilities`, `interest_expense`
  - XBRL: `tangible_asset_acquisition`, `intangible_asset_acquisition`, `cfo`, `equity`, `cash_and_cash_equivalents`, `short_term_borrowings`, `current_portion_of_long_term_borrowings`, `long_term_borrowings`, `bonds`, `lease_liabilities`
...
- **주요 메서드**:
  ...
  - `filter_context()`: contextRef 필터링 (당기 연간 연결재무제표만 채택, dFY/eFY 모두 허용)
