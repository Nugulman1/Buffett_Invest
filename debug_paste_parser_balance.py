#!/usr/bin/env python
"""
재무상태표 파싱 테스트: 연도 + 표 본문 + rows JSON 출력.

사용법 (프로젝트 루트에서):
  python debug_paste_parser_balance.py [재무상태표.txt]
  파일 안 주면 balance.txt 참고.
"""
import json
import os
import sys
...
def run():
    ...
    path = args[0] if args else "balance.txt"
    ...
    # 2) 표 본문(자산부터) 분리 결과 txt 출력
    print("=" * 60)
    print("2. 표 본문 (자산 줄부터 분리된 txt)")
    print("=" * 60)
    ...
    print("=" * 60)
    result = parse_balance_sheet(text)
